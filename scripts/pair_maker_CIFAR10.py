...
class PairMaker:
    ...
    def __init__(self, dataset_name="MNIST", root="./data", patch_size=2, batch_size=100):
        ...
        self.smooth = False #only applied to RGB origined data
        ...
        self.contrast     = 0.3
        self.saturation   = 0.3
        # Hue jitter, in turns (0.125 = 45 deg), RGB only. 0 for every grayscale
        # dataset — the branch is skipped at C == 1, so digits are untouched.
        self.hue          = 0.0
        # Per-image demeaning applied after greyscale, before Sobel (IIC's
        # config.per_img_demean). OFF by default.
        self.per_img_demean = False
        self.min_edge     = 24
        ...
        # Grayscale conversion (CIFAR10). When True, an RGB batch is collapsed to a
        # single luma channel BEFORE chessboard masking, so colour — the cue the
        # network otherwise latches onto, clustering CIFAR by sky/foliage/background
        # rather than by object — is simply not present in its input. Edges and
        # shape are preserved. Two properties matter:
        #   * it is PER-PIXEL (each output pixel is a function of that pixel's RGB
        #     only), so the two complementary views stay exactly independent under
        #     the i.i.d.-noise null H0 — unlike a Sobel filter, whose 3x3 window
        #     straddles both chessboard parities and leaks between the views;
        #   * the result is a [0,1] intensity image, so masked positions keep their
        #     usual meaning (0 = black = "nothing here") and the rest of the
        #     pipeline — masking, rotation padding, brightness/contrast — needs no
        #     change at all.
        # OFF for the already-grayscale digit/Fashion datasets (no-op there anyway).
        self.to_grayscale = False
        # Sobel colour-decomposition, applied PER VIEW *after* chessboard masking
        # and smoothing (see _make_pairs). Order matters, in two ways:
        #   * H0 stays exact. Each smoothed view is a function of its OWN parity's
        #     pixels only, and Sobel of a function of X_A is still a function of
        #     X_A — so the two views remain independent under the i.i.d.-noise null.
        #     (Applying Sobel to the FULL image before masking, as an earlier
        #     version did, leaks: a 3x3 window straddles both parities.)
        #   * It is only meaningful after smoothing. Sobel on a half-zeroed view
        #     would measure gradients against the chessboard holes rather than
        #     against image content.
        # Motivation: the network otherwise clusters CIFAR by global BRIGHTNESS
        # LAYOUT — measured on a trained run, cluster identity carried more mutual
        # information with centre-vs-border luminance (NMI 0.087) than with the
        # ground-truth class (0.068). Sobel is a derivative operator, so it removes
        # exactly that: mean luminance retains 1.9% of its spread and the
        # centre-vs-border layout 7.9%, fully decorrelated from the original.
        # Sobel makes C == 2, so in_channels must be 2 (handled in main.py).
        ...
        if dataset_name == 'CIFAR10':
            # CIFAR10: convert to grayscale, then use exactly the same augmentation
            # policy as MNIST/FashionMNIST (class defaults: rot_val=20, rot_prob=1.0,
            # brightness/contrast=0.3, square crop from {min_edge..S}, crop_prob=1.0,
            # zoom-out OFF). Horizontal flip stays ON, as for FashionMNIST, since
            # CIFAR is not a digit dataset. Note S=32 here (vs 28 for MNIST), so the
            # default min_edge=24 gives a slightly wider crop range, {24..32}.
            # Pipeline: luma -> chessboard mask -> smooth each view -> SOBEL each
            # view -> augmentation (crop / flip / brightness / contrast; rotation
            # OFF). Grayscale first is not strictly required (Sobel collapses
            # channels internally) but it makes the smoothing step operate on one
            # channel instead of three, and keeps the intermediate views viewable.
            # Order follows IIC: augment the RGB image first, then greyscale, then
            # Sobel. Masking/smoothing are ours and come first, on RGB.
            #   RGB -> chessboard mask -> smooth -> crop/flip/brightness/contrast/
            #   saturation/hue -> greyscale (luma) -> Sobel -> (N,2,H,W)
            # Photometric values are IIC's ColorJitter(0.4, 0.4, 0.4, hue=0.125).
            # Rotation stays OFF (its zeros-padding put a high-contrast black frame
            # in the views, giving an augmented-vs-unaugmented std shift 26x the
            # digit datasets'), and min_edge=27 reproduces MNIST's 24/28 crop ratio
            # at S=32.
            print("CIFAR10: mask -> smooth -> augment(RGB, incl. saturation+hue) -> greyscale -> Sobel; no rotation")
    ...
    # ── Sobel colour-decomposition (IIC / DeepCluster) ─────────────────────────
    def _apply_sobel(self, images: torch.Tensor) -> torch.Tensor:
        """
        Convert an RGB batch to a 2-channel [Sobel-x, Sobel-y] gradient map, the
        colour-removal preprocessing IIC (following DeepCluster) uses on CIFAR:
        collapse RGB to a single intensity channel (uniform channel mean), then apply
        the horizontal and vertical Sobel kernels. This discards colour — a trivial
        clustering cue on CIFAR — and keeps edge/shape structure.

        The signed gradients (~[-4, 4] for a [0,1] input) are rescaled to [0,1] with
        0.5 = zero gradient, so the result is compatible with the rest of the pipeline,
        which assumes [0,1] data (the brightness/contrast jitter clamps to [0,1], and
        chessboard masking multiplies by 0/1).

        IMPORTANT (placement): this is applied PER VIEW, after chessboard masking and
        neighbour smoothing — NOT to the full image beforehand. Two consequences:
          * H0 is preserved exactly. The smoothed view is a function of its own
            parity's pixels only, so its Sobel transform is too, and the two views
            stay independent under the i.i.d.-noise null. (Applying Sobel before
            masking leaks, because a 3x3 window straddles both parities.)
          * Smoothing must run first. On a raw half-zeroed view, Sobel would measure
            the chessboard holes rather than image content.

        Args:
            images : (N, C, H, W) RGB (or already-grayscale) batch in [0,1].
        Returns:
            (N, 2, H, W) Sobel gradient map in [0,1].
        """
        assert images.shape[1] == 1, (
            f"_apply_sobel expects a single-channel (already greyscale) input, got "
            f"C={images.shape[1]}. Sobel runs AFTER the greyscale step."
        )
        device, dtype = images.device, images.dtype
        # IIC's kernels, verbatim (sobel1 = d/dx, sobel2 = d/dy).
        ...
        # NO scaling, clamping or re-centering — exactly as IIC's sobel_process,
        # which uses the raw conv output. An earlier version here squashed the
        # gradients into [0,1] via (edges/4).clamp(-1,1)*0.5+0.5, which compressed
        # the signal to std 0.089 (vs 0.23 for the input image) and made collapsed
        # initialisations ~5x more likely. Raw output keeps the full dynamic range.
        return F.conv2d(images, kernel, padding=1)                    # (N,2,H,W), ~[-4,4]

    def _maybe_sobel(self, images: torch.Tensor) -> torch.Tensor:
        """Apply Sobel colour-decomposition iff enabled (CIFAR10). Exact identity
        (returns the same tensor) for the grayscale digit/Fashion datasets, so their
        pipeline is unchanged."""
        if not self.use_sobel:
            return images
        return self._apply_sobel(images)


    # ── Grayscale conversion (CIFAR10) ────────────────────────────────────────
    def _maybe_grayscale(self, images: torch.Tensor) -> torch.Tensor:
        """
        Collapse an RGB batch to a single luma channel iff enabled (CIFAR10),
        using the standard ITU-R BT.601 weights (0.299 R, 0.587 G, 0.114 B) — the
        same conversion torchvision's `rgb_to_grayscale` applies. Luma is used
        rather than a flat channel mean because it tracks perceived brightness and
        so preserves edge contrast better on natural images.

        Applied to the FULL image BEFORE chessboard masking. Because it is a
        per-pixel map, each output pixel depends only on the RGB of that same
        pixel — so the two complementary views remain functions of disjoint pixel
        sets and stay exactly independent under H0. (This is the key difference
        from Sobel, which mixes neighbouring pixels across both parities.)

        Exact identity (returns the same tensor) when disabled or when the input is
        already single-channel, so the digit/Fashion pipelines are unchanged.

        Args:
            images : (N, C, H, W) batch in [0,1].
        Returns:
            (N, 1, H, W) luma image if enabled and C == 3, else `images` unchanged.
        """
        if not self.to_grayscale or images.shape[1] != 3:
            return images
    ...
    # ── Batched per-image hue rotation (torchvision ColorJitter semantics) ────
    @staticmethod
    def _adjust_hue_batch(x: torch.Tensor, hue: torch.Tensor) -> torch.Tensor:
        """
        Shift the hue of each image in an RGB batch by its own offset.

        torchvision's `ColorJitter(hue=...)` — which IIC uses with hue=0.125 — goes
        RGB -> HSV, adds an offset to H (mod 1), and converts back. `TF.adjust_hue`
        only accepts a SCALAR offset, so applying it per image would need a Python
        loop over the batch (far too slow: this runs on every ES batch, twice).
        This is the same algorithm, vectorised, with a per-image offset.

        Args:
            x   : (N, 3, H, W) in [0, 1].
            hue : (N,) offsets in turns (0.125 = 45 degrees of hue rotation).
        Returns:
            (N, 3, H, W) in [0, 1].
        """
        r, g, b = x[:, 0], x[:, 1], x[:, 2]
        ...
        def pick(c0, c1, c2, c3, c4, c5):
            out = c0.clone()
            for idx, cand in enumerate((c1, c2, c3, c4, c5), start=1):
                out = torch.where(i == idx, cand, out)
            return out

        out_r = pick(v, q, p, p, t, v)
        out_g = pick(t, v, v, q, p, p)
        out_b = pick(p, p, t, v, v, q)
        return torch.stack([out_r, out_g, out_b], dim=1).clamp(0, 1)
    ...
    def _augment_batch(
        ...
        # Augmentation now always runs on IMAGE-space data in [0,1] (greyscale and
        # Sobel both happen afterwards, matching IIC's order), so plain
        # multiplicative brightness is correct again — no neutral-level correction
        # is needed the way it was when Sobel ran before augmentation.
        if brightness > 0:
            b = torch.FloatTensor(N, 1, 1, 1).uniform_(1 - brightness, 1 + brightness).to(device)
            x = (x * b).clamp(0, 1)

        def _luma_of(t):
            """Luma image, matching torchvision's `rgb_to_grayscale` (and PIL's
            `convert('L')`, which IIC reaches via ColorJitter). torchvision's
            adjust_contrast and adjust_saturation both blend against the LUMA
            image, not a flat channel mean; using the flat mean put saturation
            0.069 and contrast 0.002 away from the reference, and disagreed with
            `_maybe_grayscale`, which already uses these weights. Recomputed per
            operation, as torchvision does, since each op mutates x.

            Returns its input unchanged for single-channel data (rgb_to_grayscale
            is the identity there), so the digit paths stay bit-for-bit unchanged.
            """
            if t.shape[1] != 3:
                return t
            w = torch.tensor([0.299, 0.587, 0.114], device=t.device, dtype=t.dtype).view(1, 3, 1, 1)
            return (t * w).sum(dim=1, keepdim=True)

        # contrast: scale deviation from the per-image mean of the luma image
        if contrast > 0:
            c = torch.FloatTensor(N, 1, 1, 1).uniform_(1 - contrast, 1 + contrast).to(device)
            mean = _luma_of(x).mean(dim=[1, 2, 3], keepdim=True)
            x = ((x - mean) * c + mean).clamp(0, 1)

        # saturation: only meaningful for RGB; skip for grayscale (C==1). Now LIVE
        # for CIFAR, because augmentation runs before the greyscale step — a
        # saturation change alters which colours map to which luma value, so it
        # survives into the greyscale/Sobel representation. (When greyscale ran
        # first, this branch was dead and the augmentation was silently lost.)
        if C == 3 and saturation > 0:
            s = torch.FloatTensor(N, 1, 1, 1).uniform_(1 - saturation, 1 + saturation).to(device)
            gray = _luma_of(x).expand_as(x)
            x = (gray + (x - gray) * s).clamp(0, 1)

        # hue: RGB only, per-image offset in turns. IIC uses ColorJitter(hue=0.125).
        # Like saturation, a hue rotation changes the luma of coloured regions, so
        # it survives greyscaling — it is one of the few augmentations that
        # decorrelates the two views WITHOUT perturbing spatial layout, so it does
        # not trade against the cross-view agreement the surprise score needs.
        if C == 3 and hue > 0:
            hue_shift = torch.FloatTensor(N).uniform_(-hue, hue).to(device)
            x = self._adjust_hue_batch(x, hue_shift)
    ...
    def _make_pairs(
        ...
        # ── Post-augmentation: greyscale -> (per-image demean) -> Sobel ──────────
        # This mirrors IIC's order exactly: all geometric and photometric
        # augmentation happens on the RGB image, and only THEN is the image
        # converted to greyscale and passed through sobel_process. Two consequences
        # that motivated moving these steps here from before the mask:
        #   * the saturation and hue jitter become effective. They act on RGB and
        #     change which colours map to which luma value, so their effect
        #     survives greyscaling. With greyscale first they were dead code.
        #   * augmentation operates on image-space data in [0,1], so multiplicative
        #     brightness and the [0,1] clamps mean what they normally mean, instead
        #     of distorting a signed gradient map.
        # H0 is unaffected: every step here is applied to each view separately, and
        # each view is already a function of its own parity's pixels only.
        # All three are exact no-ops for the digit datasets (C == 1 so greyscale
        # returns unchanged, per_img_demean and use_sobel are False).
        def _to_network_input(t):
            t = self._maybe_grayscale(t)
            if self.per_img_demean:
                t = t - t.mean(dim=(1, 2, 3), keepdim=True)
            if self.use_sobel:
                t = self._apply_sobel(t)
            return t

        tensor_i = _to_network_input(tensor_i)
        tensor_j = _to_network_input(tensor_j)
    ...
    def split_patches_from_loader(self, split="train", device=None, force_augment=None, smooth=None):
        """
        ...
        for images, labels in loader:
            tensor_i, tensor_j = self._make_pairs(images, augment=augment, device=device, smooth=self.smooth if smooth is None else smooth)
