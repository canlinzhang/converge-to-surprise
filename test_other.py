...
from scripts.pair_maker_CIFAR10 import PairMaker   # same pipeline as main_CIFAR10.py / main_softmax*.py
from scripts.utils import cluster_acc, best_cluster_fit

'''
Evaluation for everything outside the main paper results, i.e. models trained by
main_CIFAR10.py, main_softmax.py or main_softmax_2.py. Identical to test.py except
that it builds views with scripts/pair_maker_CIFAR10.py (the pipeline those scripts
train on), so CIFAR10 models get their 2-channel Sobel views. For MNIST /
FashionMNIST / USPS the views are bit-identical to test.py's.

To run
CUDA_VISIBLE_DEVICES=x python test_other.py \
    --save_dir ./model_bin/xxxx/0/ \
    --dataset_name MNIST or FashionMNIST or USPS or CIFAR10
'''
